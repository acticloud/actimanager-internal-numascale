...
class HospitalResident(BaseSolver):
    ...
    def __init__(self, suitors, reviewers):
        ...
        BaseSolver().__init__(suitors, reviewers)
    ...
    def _check_suitor_matching(self):
        ...
        for suitor in self.suitors:
            if (
                ...
                errors.append(
                    ValueError(
                        "{suitor} is matched to {suitor.matching} but they do "
    ...
    def _check_reviewer_capacity(self):
        ...
        for reviewer in self.reviewers:
            if len(reviewer.matching) > reviewer.capacity:
                errors.append(
                    ValueError(
                        "{reviewer} is matched to {reviewer.matching} which is"
                        "over their capacity of {reviewer.capacity}."
                    )
                )
        ...
        for reviewer in self.reviewers:
            for suitor in reviewer.matching:
                if suitor.name not in reviewer.pref_names:
                    errors.append(
                        ValueError(
                            "{reviewer} has {suitor} in their matching but "
    ...
    def _check_suitor_prefs(self):
        ...
        for suitor in self.suitors:
            if not set(suitor.pref_names).issubset(set(reviewer_names)):
                errors.append(
                    ValueError(
                        "{suitor} has ranked a non-reviewer: "
                        "{set(suitor.pref_names)} != {set(reviewer_names)}"
                    )
                )
    ...
    def _check_reviewer_prefs(self):
        ...
        for reviewer in self.reviewers:
            ...
            if set(reviewer.pref_names) != set(suitors_that_ranked_names):
                errors.append(
                    ValueError(
                        "{reviewer} has not ranked all the suitors that ranked"
                        " it: {set(reviewer.pref_names)} != "
                        "{set(suitors_that_ranked_names)}."
                    )
                )
